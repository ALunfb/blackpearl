...
- **Backup accounts available:** the user has 2 additional CSFloat accounts in new-account cooldown. When they clear, **rotate the Droplet IP first, then use a fresh account** — never probe a fresh account from the existing Droplet's IP. CSFloat's error message indexes abuse against IPs, and probing from a dirty IP risks flagging the fresh account on its first request. Test sequence: (1) spin up a new VPS at a different IP (different region preferred, different provider acceptable), (2) clone the repo and configure `.env` with backup account #1's API key, (3) run the single-request probe from the new VPS, (4) if 200, migrate cron to the new VPS and resume the phased fetch plan from the very slowest cadence. Hold backup account #2 in reserve in case #1 burns.
- We will never know whether the flag was account-level, IP-level, or both — we don't need to. Goal is a working site, not a diagnosis. Always isolate both variables when testing.
